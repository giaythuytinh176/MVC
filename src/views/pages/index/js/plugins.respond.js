...
SEMICOLON.widget.respondInit = function ($respondEl) {

    $respondEl = $respondEl.filter(':not(.customjs)');

    if ($respondEl.length < 1) {
        return true;
    }
    ...
    jRes.addFunc([
        {
            breakpoint: 'desktop',
            enter: function () {
                $respondEl.addClass('device-xl');
            },
            exit: function () {
                $respondEl.removeClass('device-xl');
            }
        }, {
            breakpoint: 'laptop',
            enter: function () {
                $respondEl.addClass('device-lg');
            },
            exit: function () {
                $respondEl.removeClass('device-lg');
            }
        }, {
            breakpoint: 'tablet',
            enter: function () {
                $respondEl.addClass('device-md');
            },
            exit: function () {
                $respondEl.removeClass('device-md');
            }
        }, {
            breakpoint: 'handheld',
            enter: function () {
                $respondEl.addClass('device-sm');
            },
            exit: function () {
                $respondEl.removeClass('device-sm');
            }
        }, {
            breakpoint: 'smallest',
            enter: function () {
                $respondEl.addClass('device-xs');
            },
            exit: function () {
                $respondEl.removeClass('device-xs');
            }
        }
    ]);
